...
- `conversation/state.py`: messages, loaded_traces, tool_calls, evidence, focused_metric 저장
- `analysis/tools.py`: `load_traces`, `summarize_findings`, `get_finding`, `get_evidence`, `explain_gate`, `recommend_fix`, `compare_runs`
- `conversation/agent.py`: tool-based planner/response runtime
...
- `llm/clients.py`는 stdlib 기반 provider abstraction과 `.env` loader를 제공한다.
