...
class CliTests(unittest.TestCase):
    def test_run_fixture_creates_trace_and_report(self):
        with tempfile.TemporaryDirectory() as td:
            proc = subprocess.run([sys.executable, '-m', 'reference_agent.weblog_agent.cli', 'run-fixture', 'normal-login-error-spike', '--output-dir', td], capture_output=True, text=True)
