...
def build_report(state) -> str:
    ...
    likely = "- Hypothesis: upstream service or dependency instability may be contributing; verify with service-specific logs." if anomalies else "- No likely cause identified because no anomaly was detected."

    return f"""## Summary

Analyzed web logs for `{target}`. Request count was {metrics.get('request_count', 0)} and error rate was {metrics.get('error_rate', 0.0):.2%}.
...
- Confidence: {'medium' if anomalies else 'low'} based on available fixture logs and computed metrics.
